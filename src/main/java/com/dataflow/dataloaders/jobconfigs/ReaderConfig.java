...
//    private List<AdditionalQuery> additionalQueries;
    ...
    public enum SPLIT_ROW_OPTION {
        DUPLICATE_ROW_PER_JSON_ARRAY_ENTRY //used to extract multiple rows based on an array which is stored within a JSON field
    }
    ...
    public enum DateBindType {
        TODAY, TODAY_LOCALDATE, CURRENT_DAY_INT, YESTERDAY, YESTERDAY_LOCALDATE, YESTERDAY_STRING, YESTERDAY_INT, TOMORROW, TOMORROW_LOCALDATE, VARIABLE_DATE,
        SPLIT_DATE, SPLIT_DATE_HOUR, CURRENT_HOUR_INT, CURRENT_HOUR_TIMESTAMP, LAST_HOUR_INT, LAST_HOUR_TIMESTAMP, LAST_WEEK, CURRENT_MONTH_INT, SPLIT_DATE_TODAY,
        CURRENT_MONTH_START, LAST_MONTH_INT, LAST_MONTH_START, CURRENT_YEAR_INT, LAST_YEAR_INT ,LAST_N_DAYS , DAY_BEFORE_YESTERDAY_LOCALDATE, SPLIT_DATE_PREVIOUS_HOUR , NEXT_N_DAYS
    }
    ...
    /*
     For Cassandra Reader when setting up DateBindTypes with a zoneId aka timezone, list of all timezone strings below
   ...
   // private List<RedisOperation> redisOperations;
    ...
    //because LinkTokenizer will treat entire row as one column since there isn't an ending quote,
    //use setQuoteCharacter to '@' or any other character not used within that file to prevent exception
    //example: D|PIXI 4 6" 3G Android|12345, will be split into two instead of three columns D, PIXI 4 6" 3G Android|12345
    ...
    private Boolean isMsgQueueEmpty = false; // should be added in config as true to stop the reader config if msg is empty/null
...
//    private UuidRepresentation uuidRepresentation = UuidRepresentation.UNSPECIFIED;
    ...
    public String toString() {
        ...
        if (reader != null) sb.append("reader=").append(reader).append(", ");
        if (readerType != null) sb.append("readerType=").append(readerType).append(", ");
        if (readerBuilder != null) sb.append("readerBuilder=").append(readerBuilder).append(", ");
        ...
        if (selectClause != null) sb.append("selectClause=").append(selectClause).append(", ");
        if (fromClause != null) sb.append("fromClause=").append(fromClause).append(", ");
        if (whereClause != null && !whereClause.isEmpty()) sb.append("whereClause=").append(whereClause).append(", ");
